...
class EffectList extends React.Component {
    ...
    this.setState(state => ({
      collapsedEffects: {
        ...
        [effectId]:
          collapsed !== undefined
            ? collapsed
            : !state.collapsedEffects[effectId]
      }
    }));
  };
  ...
  renderEffectList(isRoot, effectIds, elems, depth = 0, prefix = "eff") {
    return effectIds.forEach((effectId, idx) => {
      ...
      const css = matchCurrentAction(this.props.state, effectId)
        ? cssMatchAction
        : null;
      ...
      if (!this.isCollapsed(effectId) && hasChildren) {
        this.renderEffectList(false, childsEffectsId, elems, depth + 1, prefix);
      }
    });
  }
  ...
  render() {
    ...
    const rootEffectIds =
      pinnedEffectId < 0 ? this.props.rootEffectIds : [pinnedEffectId];
    this.renderEffectList(pinnedEffectId < 0, rootEffectIds, elems, 0);
    return (
