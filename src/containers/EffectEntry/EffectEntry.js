...
class EffectEntry extends React.Component {
  ...
  render() {
    const { effect, isRoot, collapsed, pinned, hasChildren } = this.props;

    let pinNode;
    if (!isRoot) {
      pinNode = pinned ? (
        <div onClick={this.onUnpin}>
          <IconUnpin />
        </div>
      ) : (
        <div onClick={this.onPin}>
          <IconPin />
        </div>
      );
    }
    ...
    return (
      <EffectEntryContainer>
        <Row onMouseDown={this.onSelect}>
          ...
          <Cell>
            <Effect effect={effect} isRoot={isRoot} />
          </Cell>
        </Row>
        {isRoot ? null : (
          <Toolbar className="effect-entry-toolbar">{pinNode}</Toolbar>
        )}
        {effect.status === STATUS_PENDING ? null : (
          <Info className="effect-entry-info">{effect.time.toFixed(0)} ms</Info>
        )}
      </EffectEntryContainer>
    );
  }
...
export default connect((state, { effectId }) => {
